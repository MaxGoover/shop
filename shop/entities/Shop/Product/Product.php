...
class Product extends ActiveRecord implements AggregateRoot
{
    ...
    public function removeModification($id): void
    {
        ...
        foreach ($modifications as $i => $modification) {
            if ($modification->isIdEqualTo($id)) {
                ...
                $this->_updateModifications($modifications);
                return;
            }
        }
        throw new \DomainException('Modification is not found.');
    ...
    private function _updateModifications(array $modifications): void
    {
    ...
    private function _updatePhotos(array $photos): void
    {
        foreach ($photos as $i => $photo) {
            $photo->setSort($i);
        }
    ...
    private function _updateReviews(array $reviews): void
    {
        $amount = 0;
        $total = 0;
